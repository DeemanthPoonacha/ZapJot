...
export const PrintableItinerary: React.FC<PrintableItineraryProps> = ({
  itinerary,
}) => {
  ...
  return (
    <div className="print-container hidden print:block p-8 bg-white text-black font-sans w-full max-w-4xl mx-auto">
      {/* Printable Header */}
      <div className="border-b-2 border-slate-900 pb-4 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-6 mt-3 text-sm text-slate-600">
          <span>
            ...
            {formatDate(itinerary.endDate)} ({itinerary.totalDays} Days)
          </span>
          {itinerary.budget > 0 && (
            <span>💰 Total Budget: ${itinerary.budget.toLocaleString()}</span>
          )}
        </div>
            ...
            {day.tasks && day.tasks.length > 0 ? (
              <ul className="space-y-2">
                {day.tasks.map((task) => (
                  <li
                    key={task.id}
                    className="flex items-start justify-between text-sm py-1 border-b border-slate-100 last:border-0"
                  >
